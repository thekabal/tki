...
class Ibank
{
    ...
    public static function ibankDeposit2(\PDO $pdo_db, $lang, $langvars, $playerinfo, $amount, $account, $tkireg, $template)
    {
        ...
        if (($amount * 1) != $amount)
        {
            self::ibankError($pdo_db, $active_template, $langvars, $langvars['l_ibank_invaliddepositinput'], "ibank.php?command=deposit", $lang, $tkireg);
        }

        if ($amount == 0)
        {
            self::ibankError($pdo_db, $active_template, $langvars, $langvars['l_ibank_nozeroamount2'], "ibank.php?command=deposit", $lang, $tkireg);
        }

        if ($amount > $playerinfo['credits'])
        {
            self::ibankError($pdo_db, $active_template, $langvars, $langvars['l_ibank_notenoughcredits'], "ibank.php?command=deposit", $lang, $tkireg);
        }
        ...
        {
            self::ibankError($pdo_db, $active_template, $langvars, "<center>Error You cannot deposit that much into your bank,<br> (Max Credits Reached)</center>", "ibank.php?command=deposit", $lang, $tkireg);
        }
